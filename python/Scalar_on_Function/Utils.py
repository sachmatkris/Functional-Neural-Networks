...
# (365, 35, 3), last dims: Temperature, Precipitation and log10 Precipitation 
original_data = np.array(list_loaded['dailyAv'])
temp_log10prec_data = original_data[:,:,[0,2]].swapaxes(1,0).reshape(35,-1, order = 'F') # only temperature and log10 precipitation
regions = np.array(list_loaded['region'])
encoder = LabelEncoder()
encoder.fit(regions)
output_data = torch.tensor(encoder.transform(regions)).long()



cv_folds = kfold_cv(temp_log10prec_data)
grid = np.linspace(1, 365, 365)
structure =  {'func':[[0,365],[365,730]], 'scalar':[730,730]}
fold_idx = 0
lst = get_data_functional(structure, temp_log10prec_data, output_data, grid, cv_folds, fold_idx)

out1 = raw_fpca(lst, [3, 3])
out2 = basis_fpca(lst, [FourierBasis([1, 365], 3), FourierBasis([1, 365], 5)], [FourierBasis([1, 365], 7), FourierBasis([1, 365], 3)], [3,3])


model = Models.NN(in_d = 6, sub_hidden = [96,96], dropout = 0, num_classes = 4, device = 'cuda', model_version='advanced')
loss = nn.CrossEntropyLoss()
pytorch_trainer(model, 'NN', loss, 'classification', out2[0], out2[1], 500, device = 'cuda')
...
for fold_idx in range(len(cv_folds)):
    train_dataloader, test_dataloader = get_data_loaders(structure, temp_log10prec_data, output_data, cv_folds, fold_idx, 'AdaFNN', 4)
    model = Models.AdaFNN(structure =  {'func':[[0,365],[365,730]], 'scalar':[730,730]}, n_bases = [4, 4],
                          bases_hidden = [[24, 24], [24,24]], sub_hidden = [16, 16, 16],
                          lambda1 = 0.0, lambda2 = 0.0,
                          num_classes = 4, dropout = 0, device = device)  
    loss = nn.CrossEntropyLoss()
    results[fold_idx] = pytorch_trainer(model, 'AdaFNN', loss, 'classification', train_dataloader, test_dataloader, EPOCHS, device = 'cuda')
    """