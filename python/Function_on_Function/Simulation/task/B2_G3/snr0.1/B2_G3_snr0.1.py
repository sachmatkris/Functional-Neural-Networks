...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        results[i, fold_idx, 0] = Utils.pytorch_trainer(model_NN, 'NN', loss, train_dataloader_nn, test_dataloader_nn, EPOCHS, lr = 0.09, device = 'cuda:0')
        ...
        model_CNN = Models.CNN(inc_nodes = structure['inc_nodes'], in_dim = structure['dimensions'], out_dim = structure['dimensions'], 
                               conv_hidden_channels = [16], fc_hidden = [64], kernel_convolution = 4, kernel_pool = 4, convolution_stride = 1, pool_stride = 2,
                               device = device)
        ...
        model_LSTM = Models.LSTM(inc_nodes = structure['inc_nodes'], out_dim = structure['dimensions'],
                                 lstm_hidden = [25], fc_hidden = [32, 32, 32], num_layers = 2, bidirectional = False, device = device)
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_LSTM, 'LSTM', loss, train_dataloader_lstm, test_dataloader_lstm, EPOCHS, lr = 0.1, device = 'cuda:0')
        ...
        model_FFDNN = Models.FFDNN(inc_nodes = structure['inc_nodes'], hidden_nodes = [64, 64, 64], out_nodes = 1,
                                ...
                                lambda_weight = 0.3, lambda_bias = 1.0, device = device)   
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FFDNN, 'FFDNN', loss, train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.5, device = 'cuda:0')
        
        # FFBNN
        train_dataloader_ffbnn, test_dataloader_ffbnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, batch_size = 16)
        bases = {'input' : [BSplineBasis(n_basis = 21)], 'hidden' : BSplineBasis(n_basis = 5)}
        model_FFBNN = Models.FFBNN(bases = bases, q = structure['dimensions'], inc_nodes = structure['inc_nodes'], hidden_nodes = [16, 16, 16], out_nodes = 1,
                                    lambda_weight = 0.9, lambda_bias = 0.8, device = device) 
        results[i, fold_idx, 3] = Utils.pytorch_trainer(model_FFBNN, 'FFBNN', loss, train_dataloader_ffbnn, test_dataloader_ffbnn, EPOCHS, lr = 0.007, device = 'cuda:0')
