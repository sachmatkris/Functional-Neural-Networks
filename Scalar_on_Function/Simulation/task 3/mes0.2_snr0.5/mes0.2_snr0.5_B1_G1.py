...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_FNN = Models.FNN(structure = structure, functional_bases = [FourierBasis(n_basis = 9)],
                            phi_bases = [BSplineBasis(n_basis = 13)], sub_hidden = [32, 32, 32],
                            dropout = 0, device = device)
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.006, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [7], bases_hidden = [[32, 32, 32]], sub_hidden = [32],
                            lambda1 = 0.2, lambda2 = 0.8, dropout = 0, device = device)     
        results[i, fold_idx, 3] = Utils.pytorch_trainer(model_AdaFNN, 'AdaFNN', loss, 'regression', train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.01, device = 'cuda:0')
        ...
        train_dataloader_fpca, test_dataloader_fpca = Utils.basis_fpca(data_FPCA,  [FourierBasis(n_basis = 15)], [BSplineBasis(n_basis = 9)], [n_component])
        ...
        results[i, fold_idx, 5] = Utils.flm(data_FLM, [BSplineBasis([-2, 4], n_basis = 9)], [BSplineBasis([-2, 4], n_basis = 5)])
