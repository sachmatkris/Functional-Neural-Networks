...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_FNN = Models.FNN(structure = structure, phi_bases = [FourierBasis(n_basis = 13)],
                               sub_hidden = [64, 64, 64], dropout = 0, device = device, smoothed = False)
        results[i, fold_idx, 1] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.002, device = 'cuda:0')

        # FNN_s
        train_dataloader_fnn, test_dataloader_fnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'FNN', batch_size = 16)
        model_FNN = Models.FNN(structure = structure, functional_bases = [BSplineBasis(n_basis = 15)],
                            phi_bases = [FourierBasis(n_basis = 13)], sub_hidden = [32, 32],
        ...
        train_dataloader_fpca, test_dataloader_fpca = Utils.basis_fpca(data_FPCA,  [FourierBasis(n_basis = 15)], [BSplineBasis(n_basis = 19)], [n_component])
        ...
        results[i, fold_idx, 5] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 7)], [BSplineBasis([-2, 4], n_basis = 9)])
