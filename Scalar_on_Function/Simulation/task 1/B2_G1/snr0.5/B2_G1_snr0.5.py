...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_CNN = Models.CNN(structure = structure, conv_hidden_channels = [8, 8], fc_hidden = [16, 16, 16],
                            kernel_convolution = 4, kernel_pool = 2, convolution_stride = 1, pool_stride = 1,
                            dropout = 0, device = device)
        results[i, fold_idx, 1] = Utils.pytorch_trainer(model_CNN, 'CNN', loss, 'regression', train_dataloader_cnn, test_dataloader_cnn, EPOCHS, lr = 0.01, device = 'cuda:0')
        ...
        model_LSTM = Models.LSTM(structure = structure, lstm_hidden = [50],
                            fc_hidden = [64, 64, 64], num_layers = 1, bidirectional = False,
                            dropout = 0, device = device)
        ...
        model_FNN = Models.FNN(structure = structure, functional_bases = [BSplineBasis(n_basis = 9)], 
        ...
        # FNN_s
        train_dataloader_fnn, test_dataloader_fnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'FNN', batch_size = 16)
        model_FNN = Models.FNN(structure = structure, functional_bases = [FourierBasis(n_basis = 15)],
                            phi_bases = [BSplineBasis(n_basis = 13)], sub_hidden = [32, 32],
                            dropout = 0, device = device)
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [5], bases_hidden = [[8]], sub_hidden = [32, 32, 32],
                            lambda1 = 1.0, lambda2 = 0.0, dropout = 0, device = device)     
        ...
        results[i, fold_idx, 7] = Utils.flm(data_FLM, [BSplineBasis([-2, 4], n_basis = 5)], [BSplineBasis([-2, 4], n_basis = 9)])
        ...
        results[i, fold_idx, 8] = Utils.fnlm(X_train_basis, X_test_basis, y_train, y_test, LocalLinearRegressionHatMatrix(bandwidth = 0.97))
