...
beta, g, snr = 1, 2, 0.1
data_directory = f'Scalar_on_Function/Simulation/data/Regression/B{beta}_G{g}/snr{snr}/'
save_directory = f'Scalar_on_Function/Simulation/Regression/B{beta}_G{g}/snr{snr}/'
...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_NN = Models.NN(in_d = in_d, sub_hidden = [32, 32, 32], dropout = 0, device = device)
        results[i, fold_idx, 0] = Utils.pytorch_trainer(model_NN, 'NN', loss, 'regression', train_dataloader_nn, test_dataloader_nn, EPOCHS, lr = 0.01, device = 'cuda:0')

        # CNN
        train_dataloader_cnn, test_dataloader_cnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'CNN', batch_size = 16)
        model_CNN = Models.CNN(structure = structure, conv_hidden_channels = [8, 8], fc_hidden = [16, 16, 16],
                            kernel_convolution = 4, kernel_pool = 4, convolution_stride = 2, pool_stride = 1,
        ...
        model_LSTM = Models.LSTM(structure = structure, lstm_hidden = [25],
                            fc_hidden = [64], num_layers = 2, bidirectional = True,
                            dropout = 0, device = device)
        ...
        # FNN_s
        train_dataloader_fnn, test_dataloader_fnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'FNN', batch_size = 16)
        model_FNN = Models.FNN(structure = structure, functional_bases = [FourierBasis(n_basis = 21)],
                            phi_bases = [BSplineBasis(n_basis = 5)], sub_hidden = [32],
                            dropout = 0, device = device)
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.003, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [6], bases_hidden = [[64, 64]], sub_hidden = [32],
                            lambda1 = 0.7, lambda2 = 0.5, dropout = 0, device = device)     
        ...
        #train_dataloader_fpca, test_dataloader_fpca = Utils.basis_fpca(data_FPCA,  [BSplineBasis(n_basis = 11)], [FourierBasis(n_basis = 9)], [n_component])
        ...
        results[i, fold_idx, 7] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 9)], [BSplineBasis([-2, 4], n_basis = 5)])
        ...
        results[i, fold_idx, 8] = Utils.fnlm(X_train, X_test, y_train.numpy(), y_test.numpy(), KNeighborsHatMatrix(n_neighbors = 15))
        #results[i, fold_idx, 8] = Utils.fnlm(X_train_basis, X_test_basis, y_train, y_test, KNeighborsHatMatrix(n_neighbors = 15))
