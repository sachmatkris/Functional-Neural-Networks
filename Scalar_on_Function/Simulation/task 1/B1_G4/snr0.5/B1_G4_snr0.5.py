...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_NN = Models.NN(in_d = in_d, sub_hidden = [32], dropout = 0, device = device)
        ...
        model_CNN = Models.CNN(structure = structure, conv_hidden_channels = [8], fc_hidden = [32, 32, 32],
        ...
        model_LSTM = Models.LSTM(structure = structure, lstm_hidden = [200],
                            fc_hidden = [16], num_layers = 2, bidirectional = True,
                            dropout = 0, device = device)
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_LSTM, 'LSTM', loss, 'regression', train_dataloader_lstm, test_dataloader_lstm, EPOCHS, lr = 0.002, device = 'cuda:0')
        ...
        model_FNN = Models.FNN(structure = structure, functional_bases = [BSplineBasis(n_basis = 11)],
                            phi_bases = [BSplineBasis(n_basis = 5)], sub_hidden = [32, 32, 32],
                            dropout = 0, device = device)
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.007, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [7], bases_hidden = [[16, 16, 16]], sub_hidden = [16, 16],
                            lambda1 = 0.5, lambda2 = 0.25, dropout = 0, device = device)     
        results[i, fold_idx, 5] = Utils.pytorch_trainer(model_AdaFNN, 'AdaFNN', loss, 'regression', train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.04, device = 'cuda:0')
        ...
        results[i, fold_idx, 7] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 7)], [BSplineBasis([-2, 4], n_basis = 5)])
        ...
        results[i, fold_idx, 8] = Utils.fnlm(X_train, X_test, y_train.numpy(), y_test.numpy(), NadarayaWatsonHatMatrix(bandwidth = 0.25))
