...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_FNN = Models.FNN(structure = structure, functional_bases = [BSplineBasis(n_basis = 11)],
                            ...
                            dropout = 0, device = device)
        results[i, fold_idx, 3] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.007, device = 'cuda:0')
        ...
        results[i, fold_idx, 6] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 7)], [BSplineBasis([-2, 4], n_basis = 5)])
        ...
        results[i, fold_idx, 7] = Utils.fnlm(X_train, X_test, y_train.numpy(), y_test.numpy(), NadarayaWatsonHatMatrix(bandwidth = 0.25))
