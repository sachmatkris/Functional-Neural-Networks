...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_CNN = Models.CNN(structure = structure, conv_hidden_channels = [8, 8], fc_hidden = [16, 16, 16],
                            kernel_convolution = 4, kernel_pool = 2, convolution_stride = 2, pool_stride = 2,
        ...
        model_LSTM = Models.LSTM(structure = structure, lstm_hidden = [150],
                            fc_hidden = [64, 64], num_layers = 1, bidirectional = True,
                            dropout = 0, device = device)
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [3], bases_hidden = [[64, 64, 64]], sub_hidden = [16, 16, 16],
        ...
        train_dataloader_fpca, test_dataloader_fpca = Utils.basis_fpca(data_FPCA,  [BSplineBasis(n_basis = 11)], [BSplineBasis(n_basis = 9)], [n_component])
        model_FPCA = Models.NN(in_d = n_component, sub_hidden = [32, 32], dropout = 0, device = 'cuda', model_version = 'advanced')
        results[i, fold_idx, 5] = Utils.pytorch_trainer(model_FPCA, 'NN', loss, 'regression', train_dataloader_fpca, test_dataloader_fpca, EPOCHS, lr = 0.02, device = 'cuda')
