...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_CNN = Models.CNN(structure = structure, conv_hidden_channels = [4, 4], fc_hidden = [16, 16, 16],
        ...
        model_LSTM = Models.LSTM(structure = structure, lstm_hidden = [10],
                            fc_hidden = [64, 64, 64], num_layers = 3, bidirectional = True,
                            dropout = 0, device = device)
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_LSTM, 'LSTM', loss, 'regression', train_dataloader_lstm, test_dataloader_lstm, EPOCHS, lr = 0.05, device = 'cuda:0')

        # FNN_o
        train_dataloader_fnn, test_dataloader_fnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'FNN', batch_size = 16)
        model_FNN = Models.FNN(structure = structure, phi_bases = [BSplineBasis(n_basis = 13)], sub_hidden = [64, 64],
                                dropout = 0, device = device, smoothed = False)
        results[i, fold_idx, 3] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.01, device = 'cuda:0')

        # FNN_s
        train_dataloader_fnn, test_dataloader_fnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'FNN', batch_size = 16)
        model_FNN = Models.FNN(structure = structure, functional_bases = [BSplineBasis(n_basis = 15)],
                            phi_bases = [BSplineBasis(n_basis = 5)], sub_hidden = [32, 32],
                            dropout = 0, device = device)
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.01, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [6], bases_hidden = [[64]], sub_hidden = [32],
                            lambda1 = 0.9, lambda2 = 0.0, dropout = 0, device = device)     
        results[i, fold_idx, 5] = Utils.pytorch_trainer(model_AdaFNN, 'AdaFNN', loss, 'regression', train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.02, device = 'cuda:0')
        ...
        results[i, fold_idx, 7] = Utils.flm(data_FLM, [BSplineBasis([-2, 4], n_basis = 5)], [BSplineBasis([-2, 4], n_basis = 5)])
        ...
        #results[i, fold_idx, 8] = Utils.fnlm(X_train, X_test, y_train.numpy(), y_test.numpy(), NadarayaWatsonHatMatrix(bandwidth = 0.37))
        results[i, fold_idx, 8] = Utils.fnlm(X_train_basis, X_test_basis, y_train, y_test, LocalLinearRegressionHatMatrix(bandwidth = 0.73))
