...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        results[i, fold_idx, 0] = Utils.pytorch_trainer(model_NN, 'NN', loss, 'regression', train_dataloader_nn, test_dataloader_nn, EPOCHS, lr = 0.007, device = 'cuda:0')

        # CNN
        train_dataloader_cnn, test_dataloader_cnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'CNN', batch_size = 16)
        model_CNN = Models.CNN(structure = structure, conv_hidden_channels = [8], fc_hidden = [64, 64],
                            kernel_convolution = 8, kernel_pool = 4, convolution_stride = 1, pool_stride = 2,
                            dropout = 0, device = device)
        results[i, fold_idx, 1] = Utils.pytorch_trainer(model_CNN, 'CNN', loss, 'regression', train_dataloader_cnn, test_dataloader_cnn, EPOCHS, lr = 0.002, device = 'cuda:0')

        # LSTM
        train_dataloader_lstm, test_dataloader_lstm = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'LSTM', batch_size = 16)
        model_LSTM = Models.LSTM(structure = structure, lstm_hidden = [25],
                            fc_hidden = [64, 64], num_layers = 1, bidirectional = False,
                            dropout = 0, device = device)
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_LSTM, 'LSTM', loss, 'regression', train_dataloader_lstm, test_dataloader_lstm, EPOCHS, lr = 0.04, device = 'cuda:0')
        ...
        model_FNN = Models.FNN(structure = structure, functional_bases = [BSplineBasis(n_basis = 5)], 
                               sub_hidden = [16], dropout = 0, device = device, smoothed = False)
        results[i, fold_idx, 3] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.003, device = 'cuda:0')


        # FNN_s
        train_dataloader_fnn, test_dataloader_fnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'FNN', batch_size = 16)
        model_FNN = Models.FNN(structure = structure, functional_bases = [FourierBasis(n_basis = 11)],
                            phi_bases = [BSplineBasis(n_basis = 5)], sub_hidden = [16, 16],
                            dropout = 0, device = device)
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.004, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [7], bases_hidden = [[8]], sub_hidden = [32, 32],
                            lambda1 = 0.6, lambda2 = 0.0, dropout = 0, device = device)     
        results[i, fold_idx, 5] = Utils.pytorch_trainer(model_AdaFNN, 'AdaFNN', loss, 'regression', train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.03, device = 'cuda:0')
        ...
        results[i, fold_idx, 6] = Utils.pytorch_trainer(model_FPCA, 'NN', loss, 'regression', train_dataloader_fpca, test_dataloader_fpca, EPOCHS, lr = 0.01, device = 'cuda')

        # FLM
        data_FLM = Utils.get_data_functional(structure, X, Y, T, cv_folds, fold_idx)
        results[i, fold_idx, 7] = Utils.flm(data_FLM, [BSplineBasis([-2, 4], n_basis = 7)], [BSplineBasis([-2, 4], n_basis = 5)])
        ...
        #results[i, fold_idx, 8] = Utils.fnlm(X_train, X_test, y_train.numpy(), y_test.numpy(), NadarayaWatsonHatMatrix(bandwidth = 0.82))
        results[i, fold_idx, 8] = Utils.fnlm(X_train_basis, X_test_basis, y_train, y_test, LocalLinearRegressionHatMatrix(bandwidth = 0.23))
