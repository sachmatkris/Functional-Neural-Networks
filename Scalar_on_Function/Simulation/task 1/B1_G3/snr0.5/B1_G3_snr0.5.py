...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_NN = Models.NN(in_d = in_d, sub_hidden = [16, 16, 16], dropout = 0, device = device)
        results[i, fold_idx, 0] = Utils.pytorch_trainer(model_NN, 'NN', loss, 'regression', train_dataloader_nn, test_dataloader_nn, EPOCHS, lr = 0.03, device = 'cuda:0')

        # CNN
        train_dataloader_cnn, test_dataloader_cnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'CNN', batch_size = 16)
        model_CNN = Models.CNN(structure = structure, conv_hidden_channels = [8, 8, 8], fc_hidden = [64, 64, 64],
                            kernel_convolution = 8, kernel_pool = 4, convolution_stride = 2, pool_stride = 1,
        ...
        model_FNN = Models.FNN(structure = structure, functional_bases = [FourierBasis(n_basis = 5)],
                            phi_bases = [BSplineBasis(n_basis = 7)], sub_hidden = [32],
                            dropout = 0, device = device)
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [6], bases_hidden = [[16, 16]], sub_hidden = [16],
                            lambda1 = 0.5, lambda2 = 1.0, dropout = 0, device = device)     
        ...
        results[i, fold_idx, 8] = Utils.fnlm(X_train, X_test, y_train.numpy(), y_test.numpy(), NadarayaWatsonHatMatrix(bandwidth = 0.77))
