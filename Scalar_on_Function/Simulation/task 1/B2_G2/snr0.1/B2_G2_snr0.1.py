...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_FNN = Models.FNN(structure = structure, functional_bases = [BSplineBasis(n_basis = 7)],
                            ...
                            dropout = 0, device = device)
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.001, device = 'cuda:0')
        ...
        results[i, fold_idx, 6] = Utils.pytorch_trainer(model_FPCA, 'NN', loss, 'regression', train_dataloader_fpca, test_dataloader_fpca, EPOCHS, lr = 0.08, device = 'cuda')
        ...
        #results[i, fold_idx, 8] = Utils.fnlm(X_train_basis, X_test_basis, y_train, y_test, LocalLinearRegressionHatMatrix(bandwidth = 0.78))
