...
data_directory = f'Scalar_on_Function/Simulation/data/task 2/n{N_SAMPLES}/B{beta}_G{g}/'
save_directory = f'Scalar_on_Function/Simulation/task 2/n{N_SAMPLES}/'
...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_FNN = Models.FNN(structure = structure, phi_bases = [BSplineBasis(n_basis = 9)],
                               sub_hidden = [32, 32, 32], dropout = 0, device = device, smoothed = False)
        results[i, fold_idx, 1] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.001, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [3], bases_hidden = [[8, 8]], sub_hidden = [64],
                            lambda1 = 0.8, lambda2 = 0.0, dropout = 0, device = device)     
        ...
        train_dataloader_fpca, test_dataloader_fpca = Utils.basis_fpca(data_FPCA,  [BSplineBasis(n_basis = 15)], [FourierBasis(n_basis = 19)], [n_component])
        ...
        results[i, fold_idx, 5] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 9)], [BSplineBasis([-2, 4], n_basis = 9)])
