...
save_directory = f'Datasets/Scalar_on_Function/Simulation/task 2/n{N_SAMPLES}/'
...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_FNN = Models.FNN(structure = structure, phi_bases = [FourierBasis(n_basis = 3)],
                               sub_hidden = [16, 16, 16], dropout = 0, device = device, smoothed = False)
        ...
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.009, device = 'cuda:0')
        ...
        results[i, fold_idx, 5] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 9)], [BSplineBasis([-2, 4], n_basis = 5)])
