...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.002, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [3], bases_hidden = [[16, 16, 16]], sub_hidden = [64],
                            lambda1 = 0.6, lambda2 = 1.0, dropout = 0, device = device)     
        ...
        results[i, fold_idx, 5] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 9)], [FourierBasis([-2, 4], n_basis = 5)])
