...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        results[i, fold_idx, 0] = Utils.pytorch_trainer(model_NN, 'NN', loss, 'regression', train_dataloader_nn, test_dataloader_nn, EPOCHS, lr = 0.003, device = 'cuda:0')

        # FNN_o
        train_dataloader_fnn, test_dataloader_fnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'FNN', batch_size = 16)
        model_FNN = Models.FNN(structure = structure, phi_bases = [FourierBasis(n_basis = 3)],
                               sub_hidden = [64, 64, 64], dropout = 0, device = device, smoothed = False)
        results[i, fold_idx, 1] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.01, device = 'cuda:0')
        

        # FNN_s
        train_dataloader_fnn, test_dataloader_fnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, 'FNN', batch_size = 16)
        model_FNN = Models.FNN(structure = structure, functional_bases = [FourierBasis(n_basis = 21)],
                            phi_bases = [BSplineBasis(n_basis = 13)], sub_hidden = [64, 64, 64],
                            dropout = 0, device = device)
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.001, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [3], bases_hidden = [[32]], sub_hidden = [64],
                            lambda1 = 0.1, lambda2 = 0.0, dropout = 0, device = device)     
        ...
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FPCA, 'NN', loss, 'regression', train_dataloader_fpca, test_dataloader_fpca, EPOCHS, lr = 0.1, device = 'cuda')

        # FLM
        data_FLM = Utils.get_data_functional(structure, X, Y, T, cv_folds, fold_idx)
        results[i, fold_idx, 5] = Utils.flm(data_FLM, [BSplineBasis([-2, 4], n_basis = 5)], [BSplineBasis([-2, 4], n_basis = 9)])
