...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_NN = Models.NN(in_dim = structure['inc_nodes'] * structure['dimensions'], hidden_dim = [16, 16],
                             out_dim = structure['dimensions'], device = device)
        results[i, fold_idx, 0] = Utils.pytorch_trainer(model_NN, 'NN', loss, train_dataloader_nn, test_dataloader_nn, EPOCHS, lr = 0.003, device = 'cuda:0')
        ...
        model_CNN = Models.CNN(inc_nodes = structure['inc_nodes'], in_dim = structure['dimensions'], out_dim = structure['dimensions'], 
                               conv_hidden_channels = [4, 4, 4], fc_hidden = [32], kernel_convolution = 4, kernel_pool = 4, convolution_stride = 2, pool_stride = 1,
                               device = device)
        results[i, fold_idx, 1] = Utils.pytorch_trainer(model_CNN, 'CNN', loss, train_dataloader_cnn, test_dataloader_cnn, EPOCHS, lr = 0.001, device = 'cuda:0')
        ...
        model_LSTM = Models.LSTM(inc_nodes = structure['inc_nodes'], out_dim = structure['dimensions'],
                                 lstm_hidden = [100], fc_hidden = [64], num_layers = 1, bidirectional = True, device = device)
        ...
        model_FFDNN = Models.FFDNN(inc_nodes = structure['inc_nodes'], hidden_nodes = [64, 64, 64], out_nodes = 1,
                                ...
                                lambda_weight = 0.0, lambda_bias = 0.8, device = device)   
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FFDNN, 'FFDNN', loss, train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.2, device = 'cuda:0')
        ...
        results[i, fold_idx, 3] = Utils.pytorch_trainer(model_FFBNN, 'FFBNN', loss, train_dataloader_ffbnn, test_dataloader_ffbnn, EPOCHS, lr = 0.003, device = 'cuda:0')
