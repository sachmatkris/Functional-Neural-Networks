...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_AdaFNN, 'AdaFNN', loss, 'regression', train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.03, device = 'cuda:0')
        ...
        results[i, fold_idx, 6] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 9)], [BSplineBasis([-2, 4], n_basis = 5)])
        ...
        results[i, fold_idx, 7] = Utils.fnlm(X_train, X_test, y_train.numpy(), y_test.numpy(), KNeighborsHatMatrix(n_neighbors = 15))
        #results[i, fold_idx, 7] = Utils.fnlm(X_train_basis, X_test_basis, y_train, y_test, KNeighborsHatMatrix(n_neighbors = 15))
