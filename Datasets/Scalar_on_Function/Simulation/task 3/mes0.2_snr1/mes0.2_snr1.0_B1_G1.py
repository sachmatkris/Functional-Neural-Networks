...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        results[i, fold_idx, 1] = Utils.pytorch_trainer(model_FNN, 'FNN', loss, 'regression', train_dataloader_fnn, test_dataloader_fnn, EPOCHS, lr = 0.008, device = 'cuda:0')
        ...
        model_AdaFNN = Models.AdaFNN(structure = structure, n_bases = [6], bases_hidden = [[64, 64, 64]], sub_hidden = [32],
                            lambda1 = 0.3, lambda2 = 0.5, dropout = 0, device = device)     
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_AdaFNN, 'AdaFNN', loss, 'regression', train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.04, device = 'cuda:0')
        ...
        train_dataloader_fpca, test_dataloader_fpca = Utils.basis_fpca(data_FPCA,  [FourierBasis(n_basis = 11)], [BSplineBasis(n_basis = 19)], [n_component])
        model_FPCA = Models.NN(in_d = n_component, sub_hidden = [32, 32], dropout = 0, device = 'cuda', model_version = 'advanced')
        results[i, fold_idx, 3] = Utils.pytorch_trainer(model_FPCA, 'NN', loss, 'regression', train_dataloader_fpca, test_dataloader_fpca, EPOCHS, lr = 0.06, device = 'cuda')

        # FLM
        data_FLM = Utils.get_data_functional(structure, X, Y, T, cv_folds, fold_idx)
        results[i, fold_idx, 4] = Utils.flm(data_FLM, [FourierBasis([-2, 4], n_basis = 7)], [BSplineBasis([-2, 4], n_basis = 5)])
