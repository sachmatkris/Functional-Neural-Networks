...
for i in range(NUM_ITER):
    ...
    for fold_idx in range(len(cv_folds)):
        ...
        model_NN = Models.NN(in_dim = structure['inc_nodes'] * structure['dimensions'], hidden_dim = [32],
                             out_dim = structure['dimensions'], device = device)
        results[i, fold_idx, 0] = Utils.pytorch_trainer(model_NN, 'NN', loss, train_dataloader_nn, test_dataloader_nn, EPOCHS, lr = 0.07, device = 'cuda:0')
        ...
        model_CNN = Models.CNN(inc_nodes = structure['inc_nodes'], in_dim = structure['dimensions'], out_dim = structure['dimensions'], 
                               conv_hidden_channels = [16, 16], fc_hidden = [64, 64], kernel_convolution = 8, kernel_pool = 4, convolution_stride = 2, pool_stride = 2,
                               device = device)
        results[i, fold_idx, 1] = Utils.pytorch_trainer(model_CNN, 'CNN', loss, train_dataloader_cnn, test_dataloader_cnn, EPOCHS, lr = 0.07, device = 'cuda:0')
        ...
        model_LSTM = Models.LSTM(inc_nodes = structure['inc_nodes'], out_dim = structure['dimensions'],
                                 lstm_hidden = [100], fc_hidden = [16], num_layers = 1, bidirectional = True, device = device)
        results[i, fold_idx, 2] = Utils.pytorch_trainer(model_LSTM, 'LSTM', loss, train_dataloader_lstm, test_dataloader_lstm, EPOCHS, lr = 0.07, device = 'cuda:0')

        # FFDNN
        train_dataloader_adafnn, test_dataloader_adafnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, batch_size = 16)
        model_FFDNN = Models.FFDNN(inc_nodes = structure['inc_nodes'], hidden_nodes = [64, 64], out_nodes = 1,
                                q = {'in' : structure['inc_nodes'] * [structure['dimensions']], 'hidden' : 10, 'out' : structure['dimensions']},
                                lambda_weight = 0.0, lambda_bias = 0.59, device = device)   
        results[i, fold_idx, 4] = Utils.pytorch_trainer(model_FFDNN, 'FFDNN', loss, train_dataloader_adafnn, test_dataloader_adafnn, EPOCHS, lr = 0.1, device = 'cuda:0')
        
        # FFBNN
        train_dataloader_ffbnn, test_dataloader_ffbnn = Utils.get_data_loaders(structure, X, Y, cv_folds, fold_idx, batch_size = 16)
        bases = {'input' : [BSplineBasis(n_basis = 5)], 'hidden' : BSplineBasis(n_basis = 15)}
        model_FFBNN = Models.FFBNN(bases = bases, q = structure['dimensions'], inc_nodes = structure['inc_nodes'], hidden_nodes = [64, 64], out_nodes = 1,
                                    lambda_weight = 0.15, lambda_bias = 0.54, device = device) 
        results[i, fold_idx, 3] = Utils.pytorch_trainer(model_FFBNN, 'FFBNN', loss, train_dataloader_ffbnn, test_dataloader_ffbnn, EPOCHS, lr = 0.4, device = 'cuda:0')
